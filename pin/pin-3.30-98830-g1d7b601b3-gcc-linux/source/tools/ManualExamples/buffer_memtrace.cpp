 ...
 * Global variables
*/
UINT32 ObservationCount_perThread[MAX_THREADS];
unordered_map<ADDRINT, UINT32> MemoryUsage[MAX_THREADS];
...
VOID* BufferFull(BUFFER_ID id, THREADID tid, const CONTEXT* ctxt, VOID* buf, UINT64 numElements, VOID* v)
{
    ...
    for (UINT64 i = 0; i < numElements; i++, reference++)
    {
        ...
        if (address != 0) {
            MemoryUsage[tid][address] = (MemoryUsage[tid][address]>size)? MemoryUsage[tid][address] : size;
            ++ObservationCount_perThread[tid];
        }
    }
...
VOID Fini(INT32 code, VOID* v)
{
    ...
    for(int i=0; i<MAX_THREADS; i++){
        for(const auto &entry : MemoryUsage[i]){
        ...
        ObservationCount += ObservationCount_perThread[i];
    }
