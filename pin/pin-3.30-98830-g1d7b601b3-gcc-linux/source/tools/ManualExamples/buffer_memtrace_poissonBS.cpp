...
UINT32 sample_poisson(double lambda, THREADID tid) {
    ...
    if (tid >= MAX_THREADS) {
        cout << "Thread ID out of range: tid=" << tid << std::endl;
        exit(1);  // or assert(false)
    }
...
{
    //cout << "Entered BufferFull for tid " << tid << endl;
    ...
    //double lambda = static_cast<double>(KnobSampleReplication.Value()) / KnobNumSplits.Value();
    double lambda = static_cast<double>(KnobNumSplits.Value()) / KnobSampleReplication.Value(); 
    ...
    for (UINT64 i = 0; i < numElements; i++, reference++)
    {
        ...
        //cout << "Element address: " << address << " and size: " << size << endl; 
        ...
        constexpr int CHUNKS = 16;  // for 1024 bins
        uint64_t usedMask[CHUNKS] = {0};
        ...
        while (count < times) {
            ...
            if (!(usedMask[chunk] & mask)) {
                usedMask[chunk] |= mask;
                count++;
                MemoryUsage[tid][bin][address] = (MemoryUsage[tid][bin][address] > size)? 
                                                MemoryUsage[tid][bin][address] : size;//std::max(MemoryUsage[tid][bin][address], size);
